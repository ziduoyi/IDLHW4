...
class ASRTrainer(BaseTrainer):
    ...
    def __init__(self, model, tokenizer, config, run_name, config_file, device=None):
        ...
        self.ce_criterion = nn.CrossEntropyLoss(
            ignore_index=self.tokenizer.pad_id, label_smoothing=config['loss']['label_smoothing']
        )
    ...
    def _train_epoch(self, dataloader):
        ...
        for i, batch in enumerate(dataloader):
            ...
            with torch.autocast(device_type=self.device, dtype=torch.float16):
                ...
                # seq_out is (B, T, C), cross entropy expects (B, C, T)
                ce_loss = self.ce_criterion(seq_out.transpose(1, 2), targets_golden)
                
                
                # TODO: Calculate CTC loss if needed
                if self.ctc_weight > 0:
                    # log probs, targets, input lengths, target lengths
                    ctc_loss = self.ctc_criterion(
                        ctc_inputs['log_probs'], targets_golden, 
                        ctc_inputs['lengths'], transcript_lengths
                    )
    ...
    def _validate_epoch(self, dataloader):
        ...
        recognition_config = {
            ...
            'beam_width': 1,
            'temperature': 1.0,
            'repeat_penalty': 1.2, # prevent loops
            'lm_weight': 0.0,
            'lm_model': None
        }
        ...
        references = [r['target'] for r in results] # ground truth
        hypotheses = [r['generated'] for r in results] # generated by model
    ...
    def train(self, train_dataloader, val_dataloader, epochs: int):
        ...
        for epoch in range(self.current_epoch, self.current_epoch + epochs):
            ...
            if val_metrics['cer'] < best_val_cer:
                print(f"New best CER: {val_metrics['cer']}, old best CER: {best_val_cer}")
    ...
    def recognize(self, dataloader, recognition_config: Optional[Dict[str, Any]] = None, config_name: Optional[str] = None, max_length: Optional[int] = None) -> List[Dict[str, Any]]:
        ...
        if recognition_config is None:
            ...
            # Beam search is really only applicable to val (not train) because train uses teacher forcing
            # No need for high beam width during val, only increase during inference
            recognition_config = {
                'num_batches': 5,
                'beam_width': 1,
                'temperature': 1.0,
            ...
            for i, batch in enumerate(dataloader):
                ...
                if recognition_config['beam_width'] > 1:
                    ...
                    seqs, scores = generator.generate_beam(
                        prompts, recognition_config['beam_width'], recognition_config['temperature'], 
                    ...
                    seqs, scores = generator.generate_greedy(
                        prompts, recognition_config['temperature'], recognition_config['repeat_penalty']
                    )
